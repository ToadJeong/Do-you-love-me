...
/**
 * Bottom-sheet for a single day: lists that day's events and lets the user add
 * a new one with an optimistic update — the event appears instantly while the
 * insert happens in the background, and rolls back if the server rejects it.
 */
export function EventBottomSheet({ date, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      {/* sheet */}
      <div className="relative max-h-[85dvh] w-full overflow-y-auto rounded-t-3xl bg-white p-5 md:mx-auto md:mb-8 md:max-w-md md:rounded-3xl">
        ...
        <form onSubmit={handleAdd} className="flex flex-col gap-3">
          ...
          <textarea
            ...
            placeholder="내용 (선택)"
            rows={3}
            className="w-full resize-none rounded-xl border border-neutral-300 px-4 py-3 text-base outline-none focus:border-neutral-900"
          />
          ...
          <button
            type="submit"
            className="rounded-xl bg-neutral-900 py-3 text-base font-medium text-white transition active:scale-[0.99]"
          >
            저장
          </button>
        </form>

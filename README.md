...
5. **설정** 에서 프로필/배경 사진, 닉네임, **사귄 날짜 수정**, 초대 코드, **테마/포인트 컬러**,
   **구글 캘린더 연결**(iCal 비공개 주소) 관리
6. 홈 하단 **버킷리스트**, **채팅+공유 메모**로 소통

> **구글 캘린더 오버레이**: 설정에 각자 구글 캘린더의 "iCal 형식 비공개 주소"를 넣으면,
> 그 일정이 공유 캘린더 위에 **파란 점(읽기 전용)** 으로 얹혀 보입니다. OAuth 불필요.
